...
class TestReplaceOpsPasses(unittest.TestCase):
    ...
    def test_replace_quantized_conv1d_ncl_with_linear(self) -> None:
        ...
        w = torch.randint(-5, 5, (out_channels, in_channels, kernel_size), dtype=torch.int8)
        ...
        self.assertEqual(
            count_node(original_gm, exir_ops.edge.cadence.quantized_conv1d_ncl.per_tensor), 1
        )
    ...
    def test_replace_quantized_conv1d_nlc_with_linear(self) -> None:
        ...
        w = torch.randint(-5, 5, (out_channels, kernel_size, in_channels), dtype=torch.int8)
        b = torch.zeros(out_channels, dtype=torch.int32)
        placeholders = (x, w, b)
        args = (
            x, w, b, [1], [0], [1], 1,
        ...
        self.assertEqual(
            count_node(original_gm, exir_ops.edge.cadence.quantized_conv1d_nlc.per_tensor), 1
        )
