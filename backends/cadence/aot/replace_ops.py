...
class ReplaceTrivialConvWithLinear(RemoveOrReplacePassInterface):
    ...
    def maybe_remove_or_replace(self, node: torch.fx.Node) -> bool:
        ...
        in_tensor = get_arg(node, "input", torch.fx.Node)
        weight = get_arg(node, "weight", torch.fx.Node)
        bias = get_arg(node, "bias", torch.fx.Node)
        padding = get_arg(node, "padding", list)
        dilation = get_arg(node, "dilation", list)
